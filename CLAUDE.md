...
| 단계 | 커맨드                                                           | 설명                                      |
| ---- | ---------------------------------------------------------------- | ----------------------------------------- |
| 1    | `/test-scenarios N`                                              | 시그니처 확정 + 테스트 시나리오 도출      |
| 2    | `/tdd-red N`                                                     | 실패 테스트 작성                          |
| 3    | `/tdd-green N`                                                   | 최소 구현, 테스트 전체 통과               |
| 4    | `@ac-verifier N`                                                 | AC 충족 독립 검증 (테스트 통과 ≠ AC 충족) |
| 5    | `/tdd-refactor N`                                                | 구조 개선, 깨지면 즉시 롤백               |
| 6    | `/security-review N`                                             | 타입·보안 점검                            |
...
- 한 단계가 끝나면 다음 단계 커맨드를 한 줄로 제안한다. 예: "다음 단계: `/tdd-red 3`"
- 승인 없이 다음 단계를 자동 실행하지 않는다.
- 단계가 실패하거나 사용자가 중단하면 해당 단계를 재시도하도록 안내한다.
- 현재 어느 단계인지 불명확하면 사용자에게 확인 후 진행한다.
